...
function vc_convert_atts_to_string($atts) {
    $output = '';
    foreach($atts as $key =>$value) {$output .= ' '.$key.'="'.$value.'"';}
    return $output;
}
...
function vc_is_editor() {
    global $vc_is_editor;
    if(is_null($vc_is_editor)) {
        $vc_is_editor = isset($_GET['vceditor']) && $_GET['vceditor'] === 'true';
    }
    return $vc_is_editor;
}
...
function vc_disable_frontend($disable = true) {
    visual_composer()->inlineEditor()->disableInline($disable);
}
